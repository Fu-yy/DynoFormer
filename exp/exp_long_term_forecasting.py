...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train_count_params(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            break
        return result_str
    def train_count_figs(self, setting):
    ...
    def train(self, setting):
        ...
        path = os.path.join(self.project_path+ os.sep +  self.args.checkpoints, setting)
        if not os.path.exists(path):
            os.makedirs(path)
        ...
        best_model_path = path + os.sep + 'checkpoint.pth'
    ...
    def test(self, setting, test=0):
        ...
        if test:
            print('loading model')
            self.model.load_state_dict(torch.load(os.path.join(self.project_path+ os.sep  + 'checkpoints'+os.sep + setting +os.sep, 'checkpoint.pth')))

        preds = []
        trues = []
        folder_path = self.project_path+ os.sep  +'test_results' +os.sep+ setting
        ...
        # result save
        folder_path = self.project_path + os.sep +  'results' +os.sep+ setting
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
