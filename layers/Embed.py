...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        # do patching
        n_vars = x.shape[1]
        x = self.padding_patch_layer(x) # 32 * 7 * 96 -- 32 * 7 * 104 右侧填充8个，左侧填充0个
        x = x.unfold(dimension=-1, size=self.patch_len, step=self.stride) # 32 * 7 * 12 * 16  其中16是patch_len  stride=18
